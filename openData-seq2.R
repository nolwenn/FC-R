...
## ----age2factor----------------------------------------------------------------------------------------
med2021$age <- factor(med2021$age, levels= c( 0, 20, 60, 99), labels=c("0-19 ANS","20-59 ANS","60 ANS ET +","AGE INCONNU"))
...
bzhn05bg_count <-  bzhn05bg %>% group_by(age, sexe) %>% summarise("boites"=sum(BOITES)) %>% 
  group_by(age) %>% mutate("pourcentage"=boites/sum(boites)*100)
bzhn05bg_count
...
# Variable sexe avec Femme en référence pour correspondance de colour
 bzhn05bg$sexe <- relevel(bzhn05bg$sexe, ref = "FEMININ")
